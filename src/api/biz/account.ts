 ...
 * @param mediaCategory 必须，媒体大类ID 1-头条; 2-百度; 3-广点通; 4-快手，number
 * @param pageNum 必须，当前页，默认1，number
 * @param pageSize 必须，每页数量，默认50，number
 */
export const postGetDistributionMediaAccountListByUserIdApi = (params: {
  userId: number;
  mediaCategory: number;
  pageNum: number;
  pageSize: number;
}) => {
  return http.post<any>(PORTBiz + `/media/getDistributionMediaAccountListByUserId`, params);
};
